...
actors have:
    - task scheduling algos like runinterval()
    - worker threadpool like tokio::spawn() and the followings
    - pubsub broker channels for broadcasting messages between other actors and different parts of the app
...
async fn execute(task: Box<dyn FnMut()->()>){ tokio::spawn(async move{task()})} and also they 
...
we must pass the data throught the mpsc channel and receive it in another scope or thread
actix multithreaded based web server:
every api is an async task which is solved inside a free thread of actix server
...
server.run(apis); // it starts sending each api using mpsc sender channel into a free thread to handle request calls asyncly and concurrently
    ...
    - execute the api in a spawned free thread
...
⚈ --------- ⚈ --------- ⚈ --------- ⚈ --------- ⚈ --------- ⚈ --------- ⚈ 
 ...
 to be executed safely and without having any race conditions or deadlocks
 between threads.
