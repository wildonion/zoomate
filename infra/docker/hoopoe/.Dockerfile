...
RUN apt-get update && \
    ...
    make install && \
    cd .. && \
    cargo build --bin hoopoe --release
...
COPY --from=builder /usr/src/app/target/release/hoopoe /app/hoopoe
...
# we must copy necessary runtime files into this stage for ./hoopoe
COPY ./.env /app/.env
